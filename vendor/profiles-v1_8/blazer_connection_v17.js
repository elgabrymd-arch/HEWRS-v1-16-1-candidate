...
function create(inputs){
 const base=original.create(inputs),d=freeze(clone(root.HEWRS_BLAZER_CONNECTION_DATA));
 ...
 for(const b of Object.values(d.blazers)){const item=catalogue.blazers.find(x=>x.id===b.historyId);if(item){const f=base.features.get(b.id);Object.assign(item,{name:b.label,color:f.primary_text,pattern:f.pattern_text,_canonical:{id:b.id,source:'CP98 pinned feature record'}});}}
 ...
 function requireAssembly(s){const b=knownBlazer(s.blazerId);need(b.availability==='CONNECTED',b.id+': '+b.availability+'; no older garment or reference photograph is substituted.');need(s.shirtId===d.assembly.shirt,'No matching full-length blazer underlayer for '+s.shirtId+' in this connected source; suit inserts are not substituted.');need(Object.hasOwn(d.assembly.ties,s.state),'This existing B03 assembly has tied states only; no collar mode is invented.');return b;}
 ...
 function identity(s){validateSelection(s);return isBlazer(s)?{topwear:knownBlazer(s.blazerId).historyId,shirt:base.records[s.shirtId].history_id,tie:s.state,shoes:s.shoeId,watch:s.watchId,pants:knownPant(s.pantId).historyId}:base.historyIds(s);}
 function makeRequest(c){
  ...
  need(c.shirt==='DS001'||c.shirt==='shirt-DS001','Select DS001 explicitly for the preserved B03 assembly; no silent shirt substitution');
  need(c.tie!=='NO_TIE'&&c.tie!=='REFERENCE','The preserved B03 route does not supply that collar mode');
  ...
  const q={mode:'candidate',dressMode:'work',topwear:{mode:'item',id:b.historyId,formal:false},pants:{mode:'item',id:p.historyId},shirt:{mode:'item',id:'shirt-DS001'},shoes:{mode:'item',id:c.shoeId},executiveStyle:c.style||'AUTO',context:{occasion:c.occasion||'clinic',requiredFormality:c.formality||'any'},localDate:c.localDate,limit:15,pantBindings:[]};
  if(c.tie&&c.tie!=='ANY'){if(c.tie.startsWith('FAMILY:'))q.tie={mode:'category',category:c.tie.slice(7)};else{need(Object.hasOwn(d.assembly.ties,c.tie),'Unknown exact tie');q.tie={mode:'item',id:c.tie};}}
  if(c.watchId)q.watch={mode:'item',id:c.watchId};
  const profile=d.profiles[p.id];
  // Nulls remain null. Do not promote COLOR_ONLY_PROJECTION_REVIEW proposals to actual scores.
  if(profile?.profileId&&profile.reviewedForLocalTest===true&&profile.evidence?.source_id)q.pantBindings.push({physicalId:p.historyId,profileId:profile.profileId,reviewedForLocalTest:true,evidence:clone(profile.evidence)});
 ...
 function selectionFromOption(o,q){
  ...
  need(base.controller.verifyCachedOption(o,q,catalogue),'Stale or invalid blazer candidate');
  ...
  const active=d.profiles[p.id];need(active?.profileId&&active.reviewedForLocalTest===true,'No activated numeric profile for this physical trouser');
 ...
 function scoreSelection(s,context){validateSelection(s);if(!isBlazer(s))return base.scoreSelection(s,context);
  const profile=d.profiles[s.pantId];if(!profile?.profileId||profile.reviewedForLocalTest!==true)return {status:'mapping_required',score:null,display_score:null,candidate_eligible:false,ids:{blazer:s.blazerId,physicalPant:s.pantId,shirt:s.shirtId,tie:s.state},reason_codes:['EXACT_NUMERIC_PROFILE_BINDING_NOT_PRESENT_IN_RECOVERED_CROSSWALK'],manual_visual_available:true};
  return base.engine.evaluate({topwearId:s.blazerId,shirtId:s.shirtId,tieId:s.state,pantProfileId:profile.profileId,context,versionPolicy:'strict'});
 }
 ...
 function queryIssue(q){
  ...
  if(!b||b.availability!=='CONNECTED'||!p||q.shirt?.id!=='shirt-DS001')return 'No complete existing blazer assembly for this exact request';
  for(const supplied of q.pantBindings||[]){const active=d.profiles[p.id];if(supplied.physicalId!==p.historyId||!active?.profileId||active.profileId!==supplied.profileId||active.reviewedForLocalTest!==true)return 'A proposed trouser profile cannot be activated by a request';}
  return null;
 }
 ...
 return Object.freeze({...base,catalogue,controller,validateSelection,historyIds:identity,makeRequest,selectionFromOption,scoreSelection,createHistoryEvent,blazerConnection,assetPaths:Object.freeze({...base.assetPaths,...d.assetPaths}),limits:{...base.limits,blazer_ids:Object.keys(d.blazers),connected_blazer_ids:blazerConnection.availableIds,physical_trouser_ids:Object.keys(d.pants),connected_blazer_shirts:d.available_shirts,blazer_numerical_profile_status:'PRESERVED_SOURCE_NULLS; later 15-binding implementation not yet recovered'}});
}
