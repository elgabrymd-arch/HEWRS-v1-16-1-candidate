...
function create(canvas,connection,resolveUrl){
 ...
 function plan(s){connection.validateSelection(s);if(!connection.blazerConnection.isBlazer(s))throw Error('Blazer selection required');return {operation:'B03_EXISTING_CONTROL_WITH_REGISTERED_REPLACEMENTS',blazerId:s.blazerId,pantId:s.pantId,shirtId:s.shirtId,state:s.state,base:a.base,avatar:connection.manifest.static.avatar,jacket:a.jacket,tie:a.ties[s.state],tieMask:a.tie_mask,shirtProtection:a.ties.T001,hands:a.hands,cuffs:a.cuffs,oldTrousers:a.old_trousers,oldShoes:a.old_shoes,pants:connection.blazerConnection.knownPant(s.pantId).layer,shoes:connection.shoeLayers[s.shoeId]};}
 async function render(s){const token=++epoch,p=plan(s),list=[p.base,p.avatar,p.jacket,p.tie,p.tieMask,p.shirtProtection,...p.hands,...p.cuffs,p.oldTrousers,p.oldShoes,p.pants,p.shoes];const ims=await Promise.all(list.map(load));if(token!==epoch)return {cancelled:true};const image=desc=>ims[list.findIndex(x=>x.sha256===desc.sha256)],data=desc=>image(desc).data;
  ...
  if(token!==epoch)return {cancelled:true};ctx.putImageData(new ImageData(out,W,H),0,0);return {status:'ready',selection:structuredClone(s),source_assembly:'B03_CHECKPOINT44_CP49_CP54'};
 }
