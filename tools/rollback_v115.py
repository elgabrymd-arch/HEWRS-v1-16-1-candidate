#!/usr/bin/env python3
"""Reconstruct exact V1.14 source in a NEW folder; never touch browser data.
Refuses existing/inside-source destinations. Runs a complete hash preflight.
"""
from pathlib import Path,PurePosixPath
import argparse,hashlib,json,os,shutil,tempfile
ROOT=Path(__file__).resolve().parents[1]
BACK=ROOT/'rollback/batch10_v1_15'
def sha(p):
 h=hashlib.sha256()
 with p.open('rb')as f:
...
def plan():
 manifest=json.loads((BACK/'PACKAGE_SHA256_V114.json').read_text())
 sources=[];names=set()
...
def main():
 ...
 temp=Path(tempfile.mkdtemp(prefix='.hewrs-v114-restore-',dir=dest.parent))
 try:
  ...
  shutil.copyfile(BACK/'PACKAGE_SHA256_V114.json',temp/'PACKAGE_SHA256.json')
