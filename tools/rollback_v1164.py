#!/usr/bin/env python3
"""Reconstruct exact packaged V1.16.3 into a separate new folder.
Read-only with --check. Never accesses browser data, GitHub or a live site.
"""
from pathlib import Path,PurePosixPath
import json,hashlib,argparse,shutil
R=Path(__file__).resolve().parents[1];F=R/'rollback/picker_v1_16_4'
def sha(p):
 h=hashlib.sha256()
 with p.open('rb')as f:
...
def main():
 ...
 print(json.dumps({'status':'PASS','restored_version':'1.16.3','baseline_files':len(entries),'write_performed':bool(a.output),'browser_data_accessed':False,'current_source_changed':False},indent=2))
