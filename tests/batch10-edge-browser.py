...
R=Path(__file__).resolve().parents[1];E=R/'evidence/batch10_v1_15/EDGE_BROWSER.json';checks=[]
IDs=['DS023','DS004','DS047']
...
with sync_playwright()as pw:
 ...
 try:
  ...
  result=p.evaluate('''async ss=>{const c=HEWRSApp.connection,v=document.createElement('canvas'),bad=c.batch10.shirts.DS004.modes.no_tie.base.sha256;const r=HEWRSAtomicRenderer.create(v,c,{resolveUrl:d=>d.sha256===bad?'data:image/png;base64,INVALID':'data:image/png;base64,'+HEWRS_EMBEDDED_IMAGES[d.sha256]});await r.render(ss[0]);const before=v.toDataURL();let rejected=false;try{await r.render(ss[1]);}catch(e){rejected=true;}return {rejected,pixels_preserved:before===v.toDataURL(),last:r.last(),status:v.dataset.status};}''',selections)
...
result={'schema':'hewrs.batch10.edge-browser.v1_15','scope':'Actual browser/image decoding with memory-loaded files and synthetic storage; not hosted/restart acceptance','passed':sum(x['passed']for x in checks),'failed':sum(not x['passed']for x in checks),'checks':checks};E.write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2))
