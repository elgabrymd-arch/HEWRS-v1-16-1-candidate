...
E=Path(os.environ.get('HEWRS_V114_ORIGIN',str(R/'evidence/source_routes_v1_14/REAL_ORIGIN.json')))
...
result={'schema':'hewrs.real-origin-check.v1_14','status':'NOT_COMPLETED','scope':'Separate temporary Chromium profile, actual localhost HTTP, actual localStorage; no physical Safari/iPhone claim.', 'url':url,'checks':[],'requests':requests,'application_loaded':False,'storage_mocked':False,'policy_modified':False}
...
with tempfile.TemporaryDirectory(prefix='hewrs-v114-isolated-')as profile:
 with sync_playwright()as pw:
  ...
  try:
   ...
   page.evaluate('''()=>{localStorage.setItem('hewrs-v114-test-sentinel','UNCHANGED');const a=HEWRSApp;const s={shirtOnly:true,pantId:'PG002',shirtId:'DS014',state:'NO_TIE',shoeId:'shoe-8',watchId:null};a.store.addEvent(a.connection.createHistoryEvent(s,{id:'V114_ISOLATED_TEST',localDate:'2026-09-23',origin:'manual'}));}''')
   ...
   check('Sentinel survives process restart without modification',page.evaluate('localStorage.getItem("hewrs-v114-test-sentinel")')=='UNCHANGED')
