#!/usr/bin/env python3
"""Actual V1.16.1 scripts/images, in-memory Chromium loader, not a hosted/iPhone test."""
import os,json,time,base64,hashlib,io,traceback
from pathlib import Path
from PIL import Image,ImageDraw
from playwright.sync_api import sync_playwright
import facelift_harness as h
R=Path(__file__).resolve().parents[1];BASE=Path(os.environ['HEWRS_BASELINE_V115']);E=Path(os.environ.get('HEWRS_V1161_BROWSER',str(R/'evidence/release_v1_16_1/browser')));E.mkdir(parents=True,exist_ok=True)
checks=[];regressions=[];new_frames=[];extra_frames=[];errors=[];thumbs={};start=time.monotonic()
IDs=['DS002','DS006','DS008','DS021','DS022','DS027']
def save():
 (E/'RESULT.json').write_text(json.dumps({'schema':'hewrs.release.chromium.v1_16_1','scope':'Actual current scripts/images through in-memory loading. Synthetic Map storage, not actual hosted origin/restart/iPhone/Safari.','passed':sum(x['passed']for x in checks),'failed':sum(not x['passed']for x in checks),'checks':checks,'baseline_render_regressions':regressions,'new_render_selections':new_frames,'additional_all47_blazer_selections':extra_frames,'render_exercises':len(new_frames)+len(extra_frames),'device_certification':False,'errors':errors,'seconds':round(time.monotonic()-start,2)},indent=2)+'\n')
...
def load(p,root):h.R=root;h.load(p,storage={'v1161-sentinel':'UNCHANGED'})
...
with sync_playwright()as pw:
 ...
 try:
  load(p,R);ck('V1.16.1 loads with correct build identity and unchanged controller/facelift',p.evaluate('HEWRS_READY&&HEWRSApp.connection.implementationVersion==="1.16-batch16"&&HEWRSApp.version==="HEWRS_CONNECTED_APP_V1_16_1"&&document.title.endsWith("V1.16.1")'))
  ...
  previous_ten=['DS004','DS005','DS007','DS015','DS018','DS019','DS023','DS024','DS025','DS047']
  controls += [sel for id in previous_ten for t in ['T017','NO_TIE'] for sel in [O(id,t),B('B03',id,t)]]
  for n,s in enumerate(controls):
   ensure(p,R,[s]);ensure(old,BASE,[s]);run(p,s);run(old,s);a,b=digest(p),digest(old);regressions.append({'selection':s,'v1161_rgba_sha256':a,'v115_rgba_sha256':b,'equal':a==b})
   if a!=b:raise AssertionError('Regression '+str(s))
   if(n+1)%15==0:print('REGRESSION',n+1,flush=True);save()
  old.close();ck('127 existing suit/blazer/shirt-only frames are pixel-identical to V1.15',len(regressions)==127 and all(x['equal']for x in regressions))
  ...
  ck('456 six-shirt actual renders: 168 blazer combinations and 288 shirt-only states',len(new_frames)==456)
  extra=[B('B03',id,'T'+str(i).zfill(3))for id in IDs for i in range(1,48) if i != 17];ensure(p,R,extra)
  for s in extra:
   run(p,s);assert p.evaluate('HEWRSApp.renderer.last()')==s;extra_frames.append(s)
  ck('All 47 ties plus No Tie render under B03 for each of the six new IDs',p.evaluate('HEWRSApp.renderer.last().state')=='T047')
  sheet=Image.new('RGB',(len(IDs)*120,4*360),(225,225,225));draw=ImageDraw.Draw(sheet)
  ...
  sheet.save(E/'SIX_SHIRTS_ACTUAL_FRAMES_PHONE_SCALE.png')
  prefs(p,B());p.click('#pref-shirt');ck('Real blazer picker has 18 enabled, 32 gated, all 50 visible',p.locator('[data-item-id]').count()==50 and p.locator('[data-item-id][disabled]').count()==32);p.screenshot(path=str(E/'PICKER_390x844.png'));p.keyboard.press('Escape')
  prefs(p,O());p.click('#pref-shirt');ck('Real shirt-only picker enables the same exact 18 IDs',p.locator('[data-item-id]').count()==50 and p.locator('[data-item-id][disabled]').count()==32);p.keyboard.press('Escape')
  before=p.evaluate('HEWRSApp.ui.snapshot()');p.click('#pref-shirt');p.locator('[data-item-id="DS022"]').click();p.keyboard.press('Escape');ck('Real picker Cancel preserves all preferences',p.evaluate('HEWRSApp.ui.snapshot()')==before)
  p.click('#pref-shirt');p.locator('[data-item-id="DS022"]').click();p.click('#picker-apply');after=p.evaluate('HEWRSApp.ui.snapshot()');ck('Real picker Apply changes only the intended shirt',after['shirt']['id']=='DS022'and all(after[k]==before[k]for k in before if k!='shirt'))
  ...
  target=O('DS022','NO_TIE');run(p,target);frame=digest(p);ledger=p.evaluate('HEWRSApp.store.exportText()')
  ...
  ck('Wear confirmation records DS022, exact trousers and intentional null tie',event['items']['shirt']=={'id':'shirt-DS022'}and event['items']['tie']is None and event['canonical_selection']==target)
  ck('DS001/DS014 identical source bytes never merge their wear IDs',p.evaluate('HEWRSApp.connection.historyIds({shirtOnly:true,pantId:"PG002",shirtId:"DS014",state:"NO_TIE",shoeId:"shoe-8",watchId:null}).shirt')=='shirt-DS014')
  preview=B('B03','DS021','T017');run(p,preview);p.evaluate('HEWRSApp.showPage("outfits")');p.screenshot(path=str(E/'OUTFIT_390x844.png'))
  ...
  ck('Synthetic sentinel remains unchanged',p.evaluate('__STORAGE_MAP.get("v1161-sentinel")')=='UNCHANGED');ck('No uncaught runtime browser errors',not errors,errors)
 except Exception:
