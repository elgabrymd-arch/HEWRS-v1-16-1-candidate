...
function createCanonicalBindings({featureInputs, sourceBindings, active50Manifest, suitAliases, validateCandidate, supportedSuitIds=['S05'],canRenderSelection=null}) {
  ...
  function stateRequest(suitId, shirtId, state) {
    ...
    if(canRenderSelection)requireValue(canRenderSelection(suitId,shirtId,state),'No connected assembly for this exact suit/shirt/state');
    requireValue(state !== 'REFERENCE' || suitId === 'S05', 'REFERENCE identifies the retained S05/DS035 control only');
  ...
  function bindCandidate(option, request, catalogue) {
    ...
    const renderRequest = supportedSuitIds.includes(ids.topwear) && (!canRenderSelection||canRenderSelection(ids.topwear,ids.shirt,ids.tie===null?'NO_TIE':ids.tie)) ? stateRequest(ids.topwear,ids.shirt,ids.tie===null?'NO_TIE':ids.tie) : null;
    const representation = {combined_render_available:!!renderRequest,
      combined_render_scope:renderRequest?'Registered '+ids.topwear+' clothing; accessory display is provided by the application wrapper':'No combined cross-suit renderer bound by this adapter',
...
root.createCanonicalBindings=createCanonicalBindings;
})(globalThis);
